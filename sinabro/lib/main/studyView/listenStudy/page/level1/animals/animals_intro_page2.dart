...
class AnimalsIntroPage2 extends StatefulWidget {
  const AnimalsIntroPage2({
    super.key,
    this.selectedCharacter = ExplorerCharacter.gonyam,
    this.houseAsset =
        'assets/img/contents/studyListen/level1/animals/animal2.png',
    // 나중에 실제 파일명으로 교체 가능
    this.step0Audio =
        'assets/audio/tts/studyListen/level1/animals/animals_common1.mp3',
    this.step1Audio =
        'assets/audio/tts/studyListen/level1/animals/animals_step2.mp3',
    this.afterStep1Delay = const Duration(milliseconds: 800),
  });

  static const routeName = '/listen/animals/intro2';
  ...
  @override
  State<AnimalsIntroPage2> createState() => _AnimalsIntroPage2State();
}

class _AnimalsIntroPage2State extends State<AnimalsIntroPage2> {
  int _step = 0;
  ...
  Future<void> _playStep1ThenGoReveal() async {
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, SheepRevealPage.routeName);
      }
    });
  }
