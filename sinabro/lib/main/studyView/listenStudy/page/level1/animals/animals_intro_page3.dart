...
class AnimalsIntroPage3 extends StatefulWidget {
  const AnimalsIntroPage3({
    super.key,
    this.selectedCharacter = ExplorerCharacter.tosoom,
    this.houseAsset =
        'assets/img/contents/studyListen/level1/animals/animal3.png',
    // 나중에 실제 파일명으로 교체 가능
    this.step0Audio =
        'assets/audio/tts/studyListen/level1/animals/animals_common1.mp3',
    this.step1Audio =
        'assets/audio/tts/studyListen/level1/animals/animals_step3.mp3',
    this.afterStep1Delay = const Duration(milliseconds: 800),
  });

  static const routeName = '/listen/animals/intro3';
  ...
  @override
  State<AnimalsIntroPage3> createState() => _AnimalsIntroPage3State();
}

class _AnimalsIntroPage3State extends State<AnimalsIntroPage3> {
  int _step = 0;
  ...
  Future<void> _playStep1ThenGoReveal() async {
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, ChickenRevealPage.routeName);
      }
    });
  }
