...
class MonkeyRevealPage extends StatefulWidget {
  const MonkeyRevealPage({super.key});

  static const routeName = '/listen/animals/monkey-reveal';

  @override
  State<MonkeyRevealPage> createState() => _MonkeyRevealPageState();
}

class _MonkeyRevealPageState extends State<MonkeyRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('MonkeyReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, MonkeyStoryPage.routeName);
      }
    });
  }
