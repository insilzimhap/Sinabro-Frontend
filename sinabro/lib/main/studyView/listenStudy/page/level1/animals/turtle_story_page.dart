...
class TurtleStoryPage extends StatefulWidget {
  const TurtleStoryPage({
    super.key,
    this.onFinished,
  });

  /// 네임드 라우트가 필요하면 등록해서 써도 돼요.
  static const routeName = '/listen/animals/turtle-story';

  /// 마지막 장면 이후 이어서 갈 화면이 있다면 콜백으로 연결
  final VoidCallback? onFinished;

  @override
  State<TurtleStoryPage> createState() => _TurtleStoryPageState();
}

class _TurtleStoryPageState extends State<TurtleStoryPage> {
  ...
  Future<void> _playCurrent() async {
    ...
    try {
      ...
      debugPrint('turtleStory audio error: $e');
    }
  ...
  void _goToOutro() {
    Navigator.of(context).pushReplacement(PageRouteBuilder(
      ...
      pageBuilder: (_, __, ___) => const TurtleOutroPage(),
