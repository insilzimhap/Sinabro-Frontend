...
class ChickenRevealPage extends StatefulWidget {
  const ChickenRevealPage({super.key});

  static const routeName = '/listen/animals/chicken-reveal';

  @override
  State<ChickenRevealPage> createState() => _ChickenRevealPageState();
}

class _ChickenRevealPageState extends State<ChickenRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('ChickenReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, ChickenStoryPage.routeName);
      }
    });
  }
