...
class TurtleRevealPage extends StatefulWidget {
  const TurtleRevealPage({super.key});

  static const routeName = '/listen/animals/turtle-reveal';

  @override
  State<TurtleRevealPage> createState() => _TurtleRevealPageState();
}

class _TurtleRevealPageState extends State<TurtleRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('TurtleReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, TurtleStoryPage.routeName);
      }
    });
  }
