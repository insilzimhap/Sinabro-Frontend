// lib/main/studyView/listenStudy/animals/elephant_outro_page.dart
//
// 마지막 동물(코끼리) 엔딩 + 전체 완료 화면을 한 파일에서 순차 진행
// Step 0) 코끼리 엔딩 (배경: elephant_06.png, 말풍선 텍스트1, 오디오: elephant06.mp3)
...
class ElephantOutroPage extends StatefulWidget {
  const ElephantOutroPage({
    super.key,
    this.onFinished,
  });

  static const routeName = '/listen/animals/elephant-outro';

  /// 전 구간 재생 완료 후 호출(예: 사과 팝업 띄우기, 트리 화면 복귀 등)
  final VoidCallback? onFinished;

  @override
  State<ElephantOutroPage> createState() => _ElephantOutroPageState();
}

class _ElephantOutroPageState extends State<ElephantOutroPage> {
  ...
  static const String _frogBg =
      'assets/img/contents/studyListen/level1/animals/elephant_06.png';
  static const String _frogLine = '코끼리는 긴 코로 물을 마시러 갔어요';
  static const String _frogAudio =
      'assets/audio/tts/studyListen/level1/animals/elephant06.mp3';
  ...
  static const String _finalLine = '동물원에서 만날 수 있는\n동물 친구들을 모두 만났어요!';
  ...
  Future<void> _playCurrent() async {
    ...
    try {
      ...
      debugPrint('ElephantOutro audio error($_step): $e');
      // 오디오 실패해도 UX 흐름은 보장
    ...
    return StudyBackLayout(
      ...
      body: FigmaBoard(
        ...
        builder: (context, scale, dx, dy) {
          ...
          if (_step == 0) {
            // -------------------------------
            // STEP 0: 개구리 엔딩 화면 (배경 full + 하단 말풍선)
            // 피그마 레이아웃:
            // - 배경 이미지 cover
