// lib/main/studyView/listenStudy/animals/dog_outro_page.dart
import 'dart:async';
...
class DogOutroPage extends StatefulWidget {
  const DogOutroPage({
    super.key,
    this.onFinished,
  });

  static const routeName = '/listen/animals/dog-outro';

  /// 마지막 문구까지 재생 완료 후 호출(다음 동물 실루엣으로 이동 등에 사용)
  final VoidCallback? onFinished;

  @override
  State<DogOutroPage> createState() => _DogOutroPageState();
}

class _DogOutroPageState extends State<DogOutroPage> {
  final _player = AudioPlayer();
  StreamSubscription<PlayerState>? _sub;

  // 🔧 경로와 문구(피그마 좌표는 FigmaBoard로 스케일 처리)
  static const _bgAsset =
      'assets/img/contents/studyListen/level1/animals/dog_06.png';
  static const _stepAudios = <String>[
    'assets/audio/tts/studyListen/level1/animals/dog06.mp3',
  ...
  Future<void> _playCurrent() async {
    ...
    try {
      ...
      debugPrint('DogOutro audio error: $e');
    }
      ...
      if (s.processingState == ProcessingState.completed) {
        ...
        if (_step < _stepTexts.length - 1) {
          ...
          if (widget.onFinished == null && mounted) {
            Navigator.pushReplacementNamed(context, CatRevealPage.routeName);
          }
        }
      }
    });
