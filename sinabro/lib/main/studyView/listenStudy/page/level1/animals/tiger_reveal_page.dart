...
class TigerRevealPage extends StatefulWidget {
  const TigerRevealPage({super.key});

  static const routeName = '/listen/animals/tiger-reveal';

  @override
  State<TigerRevealPage> createState() => _TigerRevealPageState();
}

class _TigerRevealPageState extends State<TigerRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('TigerReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, TigerStoryPage.routeName);
      }
    });
  }
