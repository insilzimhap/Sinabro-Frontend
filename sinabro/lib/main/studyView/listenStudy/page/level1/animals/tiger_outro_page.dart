// lib/main/studyView/listenStudy/animals/tiger_outro_page.dart
import 'dart:async';
...
class TigerOutroPage extends StatefulWidget {
  const TigerOutroPage({
    super.key,
    this.onFinished,
  });

  static const routeName = '/listen/animals/tiger-outro';

  /// 마지막 문구까지 재생 완료 후 호출(다음 동물 실루엣으로 이동 등에 사용)
  final VoidCallback? onFinished;

  @override
  State<TigerOutroPage> createState() => _TigerOutroPageState();
}

class _TigerOutroPageState extends State<TigerOutroPage> {
  final _player = AudioPlayer();
  StreamSubscription<PlayerState>? _sub;

  // 🔧 경로와 문구(피그마 좌표는 FigmaBoard로 스케일 처리)
  static const _bgAsset =
      'assets/img/contents/studyListen/level1/animals/tiger_06.png';
  static const _stepAudios = <String>[
    'assets/audio/tts/studyListen/level1/animals/tiger06.mp3',
  ...
  Future<void> _playCurrent() async {
    ...
    try {
      ...
      debugPrint('TigerOutro audio error: $e');
    }
      ...
      if (s.processingState == ProcessingState.completed) {
        ...
        if (_step < _stepTexts.length - 1) {
          ...
          if (widget.onFinished == null && mounted) {
            Navigator.pushReplacementNamed(context, MonkeyRevealPage.routeName);
          }
        }
      }
    });
