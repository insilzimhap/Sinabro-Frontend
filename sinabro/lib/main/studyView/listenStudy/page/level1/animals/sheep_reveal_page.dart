...
class SheepRevealPage extends StatefulWidget {
  const SheepRevealPage({super.key});

  static const routeName = '/listen/animals/sheep-reveal';

  @override
  State<SheepRevealPage> createState() => _SheepRevealPageState();
}

class _SheepRevealPageState extends State<SheepRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('SheepReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, SheepStoryPage.routeName);
      }
    });
  }
