...
class FlogRevealPage extends StatefulWidget {
  const FlogRevealPage({super.key});

  static const routeName = '/listen/animals/flog-reveal';

  @override
  State<FlogRevealPage> createState() => _FlogRevealPageState();
}

class _FlogRevealPageState extends State<FlogRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('FlogReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, FlogStoryPage.routeName);
      }
    });
  }
          ...
          return Stack(
            children: [
              ...
              Positioned(
                ...
                height: u.sw(805),
                child: Image.asset(
                  'assets/img/contents/studyListen/level1/animals/flog_00.png',
                  fit: BoxFit.cover,
                ),
              ),
