...
class DuckRevealPage extends StatefulWidget {
  const DuckRevealPage({super.key});

  static const routeName = '/listen/animals/duck-reveal';

  @override
  State<DuckRevealPage> createState() => _DuckRevealPageState();
}

class _DuckRevealPageState extends State<DuckRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('DuckReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, DuckStoryPage.routeName);
      }
    });
  }
          ...
          return Stack(
            children: [
              ...
              Positioned(
                ...
                height: u.sw(805),
                child: Image.asset(
                  'assets/img/contents/studyListen/level1/animals/duck_00.png',
                  fit: BoxFit.cover,
                ),
              ),
