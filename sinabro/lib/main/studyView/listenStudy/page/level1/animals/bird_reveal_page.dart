...
class BirdRevealPage extends StatefulWidget {
  const BirdRevealPage({super.key});

  static const routeName = '/listen/animals/bird-reveal';

  @override
  State<BirdRevealPage> createState() => _BirdRevealPageState();
}

class _BirdRevealPageState extends State<BirdRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('BirdReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, BirdStoryPage.routeName);
      }
    });
  }
          ...
          return Stack(
            children: [
              ...
              Positioned(
                ...
                height: u.sw(804.12),
                child: Image.asset(
                  'assets/img/contents/studyListen/level1/animals/bird_00.png',
                  fit: BoxFit.cover,
                ),
              ),
