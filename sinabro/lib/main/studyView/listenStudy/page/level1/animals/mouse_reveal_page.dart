...
class MouseRevealPage extends StatefulWidget {
  const MouseRevealPage({super.key});

  static const routeName = '/listen/animals/mouse-reveal';

  @override
  State<MouseRevealPage> createState() => _MouseRevealPageState();
}

class _MouseRevealPageState extends State<MouseRevealPage> {
  ...
  Future<void> _playAndNavigate() async {
    try {
      ...
      debugPrint('MouseReveal audio error: $e');
    }
    ...
    _playerSub = _player.playerStateStream.listen((s) async {
      if (s.processingState == ProcessingState.completed) {
        ...
        if (!mounted) return;
        Navigator.pushReplacementNamed(context, MouseStoryPage.routeName);
      }
    });
  }
