// lib/main/studyView/listenStudy/animals/rabbit_outro_page.dart
import 'dart:async';
...
class RabbitOutroPage extends StatefulWidget {
  const RabbitOutroPage({
    super.key,
    this.onFinished,
  });

  static const routeName = '/listen/animals/rabbit-outro';

  /// 마지막 문구까지 재생 완료 후 호출(다음 동물 실루엣으로 이동 등에 사용)
  final VoidCallback? onFinished;

  @override
  State<RabbitOutroPage> createState() => _RabbitOutroPageState();
}

class _RabbitOutroPageState extends State<RabbitOutroPage> {
  final _player = AudioPlayer();
  StreamSubscription<PlayerState>? _sub;

  // 🔧 경로와 문구(피그마 좌표는 FigmaBoard로 스케일 처리)
  static const _bgAsset =
      'assets/img/contents/studyListen/level1/animals/rabbit_06.png';
  static const _stepAudios = <String>[
    'assets/audio/tts/studyListen/level1/animals/rabbit06.mp3',
    'assets/audio/tts/studyListen/level1/animals/animals_common3.mp3',
  ];
  static const _stepTexts = <String>[
    '토끼는 깡총깡총 당근을 먹으러 갔어요',
    '다음 친구를 만나러 가볼까요?',
  ];
  ...
  Future<void> _playCurrent() async {
    ...
    try {
      ...
      debugPrint('RabbitOutro audio error: $e');
    }
        ...
        if (_step < _stepTexts.length - 1) {
          ...
          if (widget.onFinished == null && mounted) {
            Navigator.pushReplacementNamed(
                context, ElephantRevealPage.routeName);
          }
        }
      }
    });
